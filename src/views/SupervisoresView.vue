  ...
  <PageLayout title="Supervisores">
    ...
    <div class="mp-card p-0 overflow-hidden">
      <div class="px-4 py-3 border-bottom d-flex align-items-center justify-content-between">
        ...
        <span class="text-muted" style="font-size:0.8rem;">{{ supervisores.length }} supervisores</span>
      </div>
      <div class="table-responsive">
        <table class="table mb-0">
          ...
          <tbody>
            ...
            <tr v-for="s in supervisores" :key="s.id">
              ...
              <td class="text-end d-flex justify-content-end gap-1">
                <button class="btn btn-sm btn-outline-primary" @click="abrirModal(s)"><i class="bi bi-pencil"></i></button>
                <button class="btn btn-sm btn-outline-danger" @click="desactivar(s.id)"><i class="bi bi-trash"></i></button>
              </td>
            </tr>
          </tbody>
        </table>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div class="mb-3">
              ...
              <input type="text" class="form-control" v-model="form.nombre" placeholder="Nombre del supervisor" />
            </div>
...
const cargar = async () => {
  cargando.value = true;
  try { const res = await api.get('/supervisores'); supervisores.value = res.data; }
...
const guardar = async () => {
  ...
  try {
    if (editando.value) await api.put(`/supervisores/${editandoId.value}`, form.value);
    else await api.post('/supervisores', form.value);
    await cargar();
    bsModal.hide();
  } catch (e) {
...
const desactivar = async (id) => {
  if (!confirm('¿Desactivar este supervisor?')) return;
